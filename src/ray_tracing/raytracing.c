...
/*   Updated: 2023/10/30 19:40:12 by rmkrtchy         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	sphere_intersection(t_cam *cam, t_vect *ray, t_sph *sph)
{
	...
	x1 = 0;
	x2 = 0;
	cam_to_sphere = substraction_vect(cam->dir, sph->coord);
	b = 2 * (dot_product_vect(cam_to_sphere, ray));
	c = dot_product_vect(cam_to_sphere,cam_to_sphere) * sph->radius;
	disc = b * b - (4 * c);
	free(cam_to_sphere);
	if (disc < 0)
		return (0);
	...
	while (y_angle >= scene->height / 2)
	{
		...
		while (x_angle >= scene->width / 2)
		{
			...
			if (sphere_intersection(scene->cam, ray, scene->sph))
				...
				color = 0;
			my_mlx_pixel_put(scene->mlx, mlx_x, mlx_y, color);
			free(ray);
			x_angle++;
			mlx_x++;
		}
