...
t_vect	norm_cone(t_cone *cone, t_vect p, float t, t_scene *scene)
{
	...
	ray_norm = norm_vect(substract_v(substract_v(p, cone->coord), num_product_vect(cone->axis, ((1 + powf(k, 2))) * m)));
	return (ray_norm);
}
...
float	cone_inter(t_vect pos, t_vect ray, t_cone *cone)
{
	...
	math.a = dot_product_vect(ray, ray) - (1 + powf(k, 2)) * dot_product_vect(ray, cone->axis) * dot_product_vect(ray, cone->axis);
	math.b = 2 * (dot_product_vect(ray, oc) - (1 +  powf(k, 2)) * dot_product_vect(ray, cone->axis) * dot_product_vect(oc, cone->axis));
	math.c = dot_product_vect(oc, oc) - (1 + powf(k, 2)) * dot_product_vect(oc, cone->axis) * dot_product_vect(oc, cone->axis);
